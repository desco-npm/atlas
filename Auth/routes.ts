...
export default ({ Express, }: ServerRouterParams) => {
  ...
  Express.put(AuthConfig.get('routes.active'), (req, res) => {
    Auth.active(req.body)
      .then(response => res.json(response))
      .catch(e => res.status(e.statusCode).json(e))
  })

  Express.put(AuthConfig.get('routes.sendRefreshPasswordCode'), (req, res) => {
    Auth.sendRefreshPasswordCode(req.body)
      .then(response => res.json(response))
      .catch(e => res.status(e.statusCode).json(e))
  })

  Express.put(AuthConfig.get('routes.refreshPassword'), (req, res) => {
    Auth.refreshPassword(req.body)
      .then(response => res.json(response))
      .catch(e => res.status(e.statusCode).json(e))
  })

  Express.put(AuthConfig.get('routes.logout'), (req, res) => {
